...
$furniture = mysqli_query($link, "SELECT * FROM `sales`");
$i = 2;
$totalCount = 0;
$objExcel->setActiveSheetIndex(0)->mergeCells('A1:G1')->setCellValue('A1', 'Продажи')->getStyle()->getFont()->setSize(20);
...
$objExcel->setActiveSheetIndex(0)->setCellValue("G$i", "Дата")->getStyle("G$i")->getFont()->setBold(true);
...
while ($fur = mysqli_fetch_assoc($furniture))
{
    ...
    $date = $fur['date'];
    $objExcel->setActiveSheetIndex(0)->setCellValue("G$i", "$date")->getColumnDimension('G')->setWidth(30);
    $i++;
}
$i = $i+1;
...
$objWriter->save('Отчет_продаж.xlsx');
